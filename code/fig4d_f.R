...
#print(subclone)
sub_neoantigen <- read.table('fig4d_f.txt',header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
...
pdf(paste("fig4d_f.pdf",sep=""),width = 20, height = 10)


library(stringr)
sub_neoantigen$Patient<- str_replace(sub_neoantigen$Sample, "[TPO]+$", "")
response <- read.table("fig4_d_f_b.txt",header=TRUE, sep="\t", na.strings="NA", dec=".", strip.white=TRUE)
merge_res <- merge(response,sub_neoantigen,by="Patient")

sub_neoantigen_TP <- merge_res[ with(merge_res,  grepl("TP", Sample)) , ]
sub_neoantigen_TO <- merge_res[ with(merge_res,  grepl("TO", Sample)) , ]



p2 <- pre_scatter(sub_neoantigen_TP, "meanCCF", "NTMB", 'Riaz-2017-Cell-Pre-therapy')



sub_neoantigen_TO <- sub_neoantigen_TO[which(sub_neoantigen_TO$Response != 'NE'),]
...
#print()
#scatter(CRPR, "meanCCF", "NTMB", 'Riaz-2017-Cell-CR/PR')i
p7 <- on_scatter(CDPD, "meanCCF", "NTMB", 'Riaz-2017-Cell-SD/PD')


CDPD_50 <- CDPD[which(CDPD$NTMB > 50),]
p10 <- on_scatter(CDPD_50, "meanCCF", "NTMB", 'Riaz-2017-Cell-SD/PD (NTMB>50)')

combined_plot <- ggarrange(p2, p7, p10, ncol = 3, nrow = 1)
print(combined_plot)
