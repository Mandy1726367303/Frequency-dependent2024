...
# accumulations, under negative frequency-dependent selection (NFDS) with subclonal immune escape (the effect of checkpoint ligand overexpression).
# This code corresponds to an ideal population/measurement, with no measurement noise (but intrinsic stochasticity) and
# user-defined detection limit for mutations.
...
cutoff = 0.5 # Immunogenicity threshold
escape_rate = 1e-4 # Subclonal immune escape rate
s = -0.8 # Negative selection intensity
mutations = {}
sumA = {}

# Define negative selection with tumor immunogenicity threshold
...
# set the death rate of tumor cells to basal death rate if cell antigenicities are below a cutoff or immune escaped
...
E = system.log['E']
N = system.log['N']
np.savetxt('p_NFDS_IE.txt', pop_size, fmt='%d', delimiter=',')
np.savetxt('t_NFDS_IE.txt', system.t, delimiter=',')
mutation_num = np.array([0, 0])
for i in mutations:
    mutation_num = np.vstack((mutation_num, [mutations[i][0], mutations[i][1]]))
mutation_num = mutation_num[1:, :]
np.savetxt('m_NFDS_IE.txt', mutation_num)
