...
mut_rate = 5.5 # Tumor mutation rate
db = 0.1 # Basal death rate
cutoff = 0.5 # Immunogenicity threshold
therapy = 25 # The time to initiate ICB therapy
...
# Tumor grow without ICB therapy
...
class System:
    def __init__(self, num_elements, inits=None, N=100, E=0, max_cell=100000):
...
# Exit simulation until a sufficient amount of time or tumor elimination
            if self.t[-1] > 50 or self.N + self.E < 1 :
                break
...
E = system.log['E']
N = system.log['N']
np.savetxt('p_NFDS_treat.txt', pop_size, fmt='%d', delimiter=',')
np.savetxt('t_NFDS_treat.txt', system.t, delimiter=',')
mutation_num = np.array([0, 0])
for i in mutations:
    mutation_num = np.vstack((mutation_num, [mutations[i][0], mutations[i][1]]))
mutation_num = mutation_num[1:, :]
np.savetxt('m_NFDS_treat.txt', mutation_num)
