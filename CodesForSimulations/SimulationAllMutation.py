...
class System:
    def __init__(self, num_elements, inits=None, N=0, E=1, max_cell=100000):
...
# The generation of mutations following a Poisson distribution
        def mutation(cell):
            new_mut_num = np.random.poisson(mut_rate)

# Assign a portion of mutations as antigenic mutations
            new_anti_num = round(0.1 * new_mut_num)
            new_mut_id = [max(mutations.keys(), default=0) + i + 1 for i in range(new_mut_num)]

# Have neutral passengers' antigenicities set to 0
            anti_val = [0 for _ in range(new_mut_num)]
            for i in range(new_anti_num):

# Antigenic mutation antigenicity sampled from a exponential distribution with mean value set to 0.2
                anti_val[i] = np.random.exponential(0.2)
...
E = system.log['E']
N = system.log['N']
np.savetxt('p_PS_clonalAnti.txt', pop_size, fmt='%d', delimiter=',')
np.savetxt('t_PS_clonalAnti.txt', system.t, delimiter=',')
mutation_num = np.array([0, 0])
for i in mutations:
    mutation_num = np.vstack((mutation_num, [mutations[i][0], mutations[i][1]]))
mutation_num = mutation_num[1:, :]
np.savetxt('m_PS_clonalAnti.txt', mutation_num)
