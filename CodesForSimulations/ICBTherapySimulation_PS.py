...
# Set the death rate of tumor cells to basal death rate if immune escaped
...
E = system.log['E']
N = system.log['N']
np.savetxt('p_PS_treat.txt', pop_size, fmt='%d', delimiter=',')
np.savetxt('t_PS_treat.txt', system.t, delimiter=',')
mutation_num = np.array([0, 0])
for i in mutations:
    mutation_num = np.vstack((mutation_num, [mutations[i][0], mutations[i][1]]))
mutation_num = mutation_num[1:, :]
np.savetxt('m_PS_treat.txt', mutation_num)
